...
```csv
timestamp,WIFI_eduroam/CH52/5260MHz/38:17:c3:c2:14:90,WIFI_HallymWiFi/CH149/5745MHz/38:17:c3:c2:14:92,...
...
```csv
timestamp,WIFI_eduroam/CH6/2437MHz/aa:bb:cc:11:22:33,WIFI_HallymWiFi/CH11/2462MHz/dd:ee:ff:44:55:66,...
...
- **(2026.07.22~) 컬럼 헤더가 `WIFI_{SSID}/CH{channel}/{freq}MHz/{mac}` 형식으로 변경됨** (스마트폰 수집 앱과 동일한 명명 규칙). 셀 값(RSSI)은 순수 숫자 그대로이며, MAC만 필요하면 헤더를 `/`로 split한 마지막 요소를 사용 — 자세한 내용은 [2026.07.22 수정사항](#20260722-수정사항) 참고
...
### 컬럼 헤더에 SSID/채널/주파수 정보 포함 — `beacon_scanner.py`, `wifi_2ghz_receiver.py`

**배경**: JPNT 논문 작성 중, 고속수집기가 스마트폰보다 추가로 탐지한 AP들이 어느 채널(특히 DFS 여부)에서 왔는지 분석하려 했으나, 기존 CSV에는 RSSI만 저장되고 채널 정보가 없어 분석이 불가능했음.

두 가지 방식을 시도했다가 폐기함:
1. RSSI 셀 값을 `"RSSI:CHANNEL"` 형식으로 합치는 방식 → **기존 후처리 코드가 깨지는 하위 호환성 문제**로 폐기
2. 채널을 `channel_...` 별도 파일로 병행 저장하는 방식 → 파일이 두 개로 나뉘는 게 번거로워 폐기

**최종 방식**: 스마트폰 수집 앱(`PDR_RF.csv`)이 이미 쓰고 있던 컬럼 헤더 명명 규칙을 그대로 따름. 셀 값(RSSI)은 순수 숫자 그대로 두고, 컬럼 헤더에 SSID/채널/주파수/MAC 정보를 전부 담는다.

```
헤더 형식: WIFI_{SSID}/CH{channel}/{freq}MHz/{mac}
예시     : WIFI_eduroam/CH11/2462MHz/24:f2:7f:ff:5d:a3
```

- **5GHz (`beacon_scanner.py`)**: `CSVWriter`가 `bssid`를 그대로 컬럼 키로 쓰던 것을, `APDataStore`에 이미 있는 `ssid`/`channel`/`freq`/`bssid`를 조합한 문자열을 컬럼 키로 사용하도록 변경.
- **2.4GHz (`wifi_2ghz_receiver.py`)**: ESP32가 시리얼로 보내는 `boardID,MAC,SSID,RSSI,Channel,SCAN` 중 그동안 버려지던 `SSID`(parts[2])와 `Channel`(parts[4])을 읽어 컬럼 키를 생성. 주파수는 2.4GHz 채널 번호로부터 계산(`2407 + 5×channel`, CH14는 2484 특수 처리).

```csv
# 예시 (2.4GHz)
timestamp,WIFI_HallymWiFi/CH11/2462MHz/24:f2:7f:ff:5d:a3,WIFI_eduroam/CH1/2412MHz/aa:bb:cc:dd:ee:02
2026-07-22 03:49:47.417,-70,
2026-07-22 03:49:47.500,,-55
```

> ⚠️ **컬럼 헤더 파싱 방식이 바뀌었습니다.** 기존에는 컬럼 헤더가 MAC 주소 하나였지만, 이제는 `WIFI_{SSID}/CH{channel}/{freq}MHz/{mac}` 형태입니다. MAC만 필요하면 `/`로 split한 마지막 요소를 쓰면 됩니다. **셀 값(RSSI) 자체는 순수 숫자 그대로**이므로, RSSI만 쓰는 기존 코드는 헤더 파싱 부분만 손보면 됩니다.
>
> 참고로 WiFi 표준 세대(WiFi4/5/6 등)는 ESP32/tcpdump가 별도로 보고하지 않아 이번 버전에는 포함하지 않았습니다. 필요하면 802.11 캡ability IE 파싱을 추가로 구현해야 합니다.

### 5GHz SSID 파싱 버그 수정 — `beacon_scanner.py`

위 헤더 인코딩 작업 후 실제 데이터로 검증하는 과정에서, **5GHz 쪽 SSID가 전부 빈 값으로 저장되는 버그**를 발견함. 원인은 `_parse_tcpdump_line()`의 SSID 정규식이 `\(([^)]*)\)\s*$` (줄 끝에 오는 괄호)을 찾도록 되어 있었는데, 실제 tcpdump beacon 출력은 `Beacon (SSID) [rates] CH: xx ...`처럼 SSID 뒤에 데이터 레이트·채널 정보가 더 이어져서 줄 끝이 아니었음. 즉 애초에 한 번도 매칭된 적이 없던 버그.

`Beacon\s*\(([^)]*)\)`로 "Beacon (" 바로 뒤 괄호를 명시적으로 찾도록 수정. 히든 SSID(빈 문자열)도 정상적으로 빈 값으로 처리됨. 2.4GHz는 ESP32 시리얼 데이터를 쓰는 별개 경로라 이 버그의 영향을 받지 않았음.
