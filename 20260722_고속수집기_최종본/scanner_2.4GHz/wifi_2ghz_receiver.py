...
출력 포맷 (Wide CSV):
  timestamp, WIFI_{SSID}/CH{channel}/{freq}MHz/{mac}, ...
  감지 안 된 AP는 빈칸
  셀 값은 순수 RSSI 정수값 (컬럼 헤더에 SSID/채널/주파수/MAC 정보 포함)
...
class WideCSVWriter:
    ...
    각 컬럼 헤더는 "WIFI_{SSID}/CH{channel}/{freq}MHz/{mac}" 형식으로,
    스마트폰 수집 앱(PDR_RF.csv)과 동일한 명명 규칙을 따른다.
    (WiFi 표준 세대(WiFi4/5/6 등)는 ESP32가 별도로 보고하지 않아 이 버전에서는 포함하지 않는다.)
    ...
    def __init__(self, filepath: str):
        ...
        self.known_cols  = []     # "WIFI_{ssid}/CH{ch}/{freq}MHz/{mac}" 문자열 목록
        self.current_ts  = None
        self.current_row = {}     # col_key -> rssi
        self.row_count   = 0
    ...
    @staticmethod
    def _channel_to_freq(channel: str) -> int:
        try:
            ch = int(channel)
    ...
    def _col_key(self, mac: str, ssid: str, channel: str) -> str:
        freq = self._channel_to_freq(channel)
        return f"WIFI_{ssid}/CH{channel}/{freq}MHz/{mac}"

    def write(self, ts: str, mac: str, rssi: str, ssid: str = '', channel: str = ''):
        with self.lock:
            col = self._col_key(mac, ssid, channel)
            is_new_col = col not in self.known_cols
            if is_new_col:
    ...
    def _flush_row(self):
        self._file.seek(0, 2)
        row = [self.current_ts] + [self.current_row.get(col, '') for col in self.known_cols]
            ...
            try:
                ...
                try:
                    ...
                    if ':' in parts[1]:
                        ...
                        rssi    = parts[3].strip()
                        channel = parts[4].strip() if len(parts) >= 5 else ''

                        collection_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
                        writer.write(collection_time, mac, rssi, ssid, channel)
