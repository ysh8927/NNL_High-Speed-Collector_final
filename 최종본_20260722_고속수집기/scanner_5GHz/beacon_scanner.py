...
class CSVWriter:
    ...
    각 셀 값 형식: "RSSI:CHANNEL" (예: "-65:52")
                  channel=0은 tcpdump 라인에서 주파수를 못 읽어 채널 미상인 경우
    """
    ...
    def write_batch(self, cycle: int, aps: List[Dict]):
        ...
        snapshot = {ap['bssid']: f"{ap['rssi']}:{ap['channel']}" for ap in aps}

        new_macs = [mac for mac in snapshot if mac not in self.known_macs]
        self.known_macs.extend(new_macs)

        self.rows.append((ts, snapshot))
        self.row_count += 1

        # 새 MAC 발견 시 또는 10행마다 파일 재작성
        if new_macs or self.row_count % 10 == 0:
            self._rewrite()
    ...
    def _rewrite(self):
        with open(self.filepath, 'w', newline='', encoding='utf-8') as f:
            ...
            for ts, snapshot in self.rows:
                row = [ts] + [snapshot.get(mac, '') for mac in self.known_macs]
                writer.writerow(row)
